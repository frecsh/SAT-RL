        ...
        try:
            if self._has_solve:
                ...
                if isinstance(legacy_result, bool):
                    ...
                    status = SolverStatus.SATISFIABLE if is_sat else SolverStatus.UNSATISFIABLE
                    ...
                    try:
                        if hasattr(legacy_result, "is_sat") or hasattr(legacy_result, "satisfiable"):
                            is_sat = getattr(legacy_result, "is_sat", getattr(legacy_result, "satisfiable", False))
                            status = SolverStatus.SATISFIABLE if is_sat else SolverStatus.UNSATISFIABLE
                        else:
                            status = SolverStatus.UNKNOWN
                            
                        solution = getattr(legacy_result, "model", getattr(legacy_result, "solution", None))
                        runtime = getattr(legacy_result, "runtime", time.time() - start_time)
                    ...
                    if hasattr(self.legacy_solver, method_name):
                        ...
                        if callable(method):
                            try:
                                ...
                                logger.error(f"Error calling legacy solver method '{method_name}': {e}")
