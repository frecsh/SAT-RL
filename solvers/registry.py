...
class SolverRegistry:
    ...
    def register(cls, name: str, solver_cls: Type[SolverBase]) -> None:
        ...
        if not issubclass(solver_cls, SolverBase):
            raise TypeError(f"Solver class {solver_cls.__name__} must inherit from SolverBase")
    ...
    @classmethod
    def get(cls, name: Optional[str] = None) -> Type[SolverBase]:
        """
    ...
    @classmethod
    def create(cls, name: Optional[str] = None, **kwargs) -> SolverBase:
        """
    ...
    def auto_discover(cls) -> None:
        ...
        for _, module_name, is_pkg in pkgutil.iter_modules([package_dir]):
            ...
            for name, obj in inspect.getmembers(module):
                if (inspect.isclass(obj) and issubclass(obj, SolverBase) and
                    obj != SolverBase and not inspect.isabstract(obj)):
