...
class WalkSATSolver(SolverBase):
    ...
    def solve(
            ...
            for _ in range(self.max_flips):
                ...
                if satisfied == len(self.clauses):
                    ...
                    return SolverResult(
                        ...
                        solution=[i+1 if lit > 0 else -(i+1) for i, lit in enumerate(assignment)],
        ...
        return SolverResult(
            status=status,
            solution=[i+1 if lit > 0 else -(i+1) for i, lit in enumerate(best_assignment)] if best_assignment else None,
            runtime=self.solve_time,
