...
class QLearningAgent:
    def __init__(self, n_vars, learning_rate=0.1, discount_factor=0.95, exploration_rate=1.0, min_exploration_rate=0.01, exploration_decay=0.995):
        ...
        # Initialize Q-table with a dictionary for sparse representation
        # Keys will be (state, action) tuples, values will be Q-values
        self.q_table = {}
    
    def get_q_value(self, state, action):
        # Convert state and action to hashable tuples
        state_tuple = tuple(state)
        action_tuple = tuple(action)
        
        # Return the Q-value if it exists, otherwise 0
...
def main(problem=None):
    ...
    agents = [QLearningAgent(n_vars=n_vars) for _ in range(n_agents)]
    ...
    print("Training multiple Q-learning agents on SAT problem...")
    ...
    for episode in range(total_episodes):
        for agent_idx, agent in enumerate(agents):
            ...
            while not done and step_count < max_steps:
                ...
                next_state, reward, done, _ = env.step(action)  # Adding the _ to accept info dict
        ...
        metrics["episode_rewards"].append(best_reward)
