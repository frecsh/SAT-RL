...
- **Agents**  
  - *Value-Based (DQN) Agent*: Learns Q-values for discrete variable selections  
  - *Actor-Critic Agent*: Manages continuous or high-level decisions (e.g., restart strategies)  
  - *Additional Agents*: Monitor unit propagation or propose sub-problem splits  
  - *Reward Modes*: Cooperative (shared reward) or competitive (individual credit)  
...
*This README outlines a research-oriented blueprint for a hybrid multi-agent RL and symbolic reasoning approach to solving SAT near its phase transition. It is intended as a living document to guide prototype development, experimentation, and eventual publication.*  
