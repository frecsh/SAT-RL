...
class BenchmarkAnalyzer:
    ...
    def analyze_time_performance(self):
        ...
        for solver in solved_df["solver"].unique():
            ...
            plt.scatter(
                solver_df["variables"], solver_df["time"], alpha=0.7, label=solver
            )
    ...
    def analyze_memory_usage(self):
        ...
        for solver in self.results_df["solver"].unique():
            ...
            plt.scatter(
                solver_df["variables"], solver_df["memory"], alpha=0.7, label=solver
            )
    ...
    def analyze_success_rates(self):
        ...
        success_by_size = (
            self.results_df.groupby(["var_bin", "solver"])["solved"]
            .mean()
            .reset_index()
        )
    ...
    def analyze_scalability(self):
        ...
        scalability_df = (
            valid_df.groupby(["solver", "variables"])["time"].mean().reset_index()
        )
    ...
    def analyze_phase_transition(self):
        ...
        plt.figure(figsize=(12, 8))
        grouped = (
            self.phase_df.groupby(["ratio", "solver"])["solved"].mean().reset_index()
        )
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(self.output_dir, f"phase_transition_success.{self.format}")
        )
        plt.close()

        # Time to solution by ratio (for solved problems)
        solved_phase = self.phase_df[self.phase_df["solved"]]

        if not solved_phase.empty:
            plt.figure(figsize=(12, 8))
            avg_times = (
                solved_phase.groupby(["ratio", "solver"])["time"].mean().reset_index()
            )

            ax = sns.lineplot(
                x="ratio", y="time", hue="solver", marker="o", data=avg_times
            )
            ...
            plt.tight_layout()
            plt.savefig(
                os.path.join(self.output_dir, f"phase_transition_time.{self.format}")
            )
            plt.close()
            ...
            for solver in avg_times["solver"].unique():
                ...
                if growth_rates:
                    ...
                    plt.bar(
                        bar_positions
                        + 0.1 * list(avg_times["solver"].unique()).index(solver),
            ...
            if growth_rates:  # Only if we have data to show
                ...
                plt.savefig(
                    os.path.join(
                        self.output_dir, f"phase_transition_growth_rate.{self.format}"
                    )
                )
                plt.close()
        ...
        for solver in grouped["solver"].unique():
            ...
            if len(solver_data) > 1:
                ...
                solver_data["success_change"] = (
                    solver_data["solved"].diff() / solver_data["ratio"].diff()
                )
        ...
        if not solved_phase.empty:
            ...
            learning_metrics = [
                col
                for col in self.phase_df.columns
                if col
                in [
                    "q_table_size",
                    "exploration_rate",
            ...
            if learning_metrics:
                ...
                for i, metric in enumerate(learning_metrics):
                    if metric in self.phase_df.columns:
                        ...
                        metric_data = (
                            self.phase_df.groupby(["ratio", "solver"])[metric]
                            .mean()
                            .reset_index()
                        )
                ...
                plt.savefig(
                    os.path.join(
                        self.output_dir,
                        f"phase_transition_learning_metrics.{self.format}",
                    )
                )
                plt.close()
    ...
    def run_statistical_tests(self):
        ...
        solved_df["problem_group"] = solved_df.apply(
            lambda x: f"{x['problem']}_{x['variables']}_{x['clauses']}", axis=1
        )
        ...
        if len(solvers) > 2:
            ...
            solver_groups = [
                solved_df[solved_df["solver"] == s]["time"] for s in solvers
            ]
            try:
        ...
        if len(solvers) > 2:
            # Prepare data for ANOVA
            solver_groups = [
                solved_df[solved_df["solver"] == s]["memory"] for s in solvers
            ]
            try:
                f_val, p_val = stats.f_oneway(*solver_groups)
        ...
        for solver1 in solvers:
            for solver2 in solvers:
                if solver1 != solver2:
                    if solver2 in stats_results["pairwise_time"].get(
                        solver1, {}
                    ) or solver1 in stats_results["pairwise_time"].get(solver2, {}):
    ...
    def create_summary_dashboard(self):
        ...
        if self.phase_df is not None:
            grouped = (
                self.phase_df.groupby(["ratio", "solver"])["solved"]
                .mean()
                .reset_index()
            )
            ax = sns.lineplot(
                x="ratio", y="solved", hue="solver", marker="o", data=grouped
            )
            ...
            plt.text(
                0.5,
                0.5,
                "Phase transition data not available",
                ha="center",
                va="center",
        ...
        print(
            f"Summary dashboard saved to {self.output_dir}/summary_dashboard.{self.format}"
        )
...
def main():
    ...
    analyzer = BenchmarkAnalyzer(
        args.results_file, args.output_dir, args.phase_file, args.format
    )
