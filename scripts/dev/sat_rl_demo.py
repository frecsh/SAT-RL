...
from src.satrlgym.agents.curriculum_sat_learner import CurriculumSATLearner
from src.satrlgym.agents.deep_q_sat_agent import DeepQSATAgent
...
def run_demo(agent_type, n_vars, n_clauses, episodes, method=None, visualize=False):
    ...
    print(
        f"Running demo with {agent_type} agent on {n_vars}-variable, {n_clauses}-clause SAT problem"
    )

    # Create logs directory
    logs_dir = f"satrlgym_logs/{agent_type}/{int(time.time())}"
    ...
    if agent_type == "dqn":
        ...
        print(
            f"Running curriculum learning from ratio {initial_ratio} to {target_ratio}..."
        )
        solution, satisfied = curriculum.solve_with_curriculum(max_attempts=5)

        print(
            f"Curriculum learning complete. Satisfied {satisfied} out of {int(target_ratio * n_vars)} clauses."
        )
        ...
        try:
            ...
            oracle = SATOracleSolver(
                n_vars=n_vars, n_clauses=n_clauses, method=method, logs_dir=logs_dir
            )
            ...
            print(
                f"Satisfied {satisfied} out of {total} clauses ({satisfied/total*100:.1f}%)"
            )
            ...
            print(
                "ERROR: Could not import SATOracleSolver. Make sure the module is available."
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--clauses", type=int, default=80, help="Number of clauses")
    parser.add_argument(
