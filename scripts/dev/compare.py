...
try:
    from src.satrlgym.agents.multi_q_sat import MultiQLearningSAT
...
try:
    from src.satrlgym.agents.multi_q_sat import MultiQLearningSATCompetitive

    def competitive_main(problem):
        # Create and run competitive multi-agent solver
        solver = MultiQLearningSATCompetitive(
            problem["num_vars"], problem["clauses"], n_agents=5
        )
        solution, metrics = solver.solve(max_episodes=300, early_stopping=True)
        return metrics
...
try:
    from src.satrlgym.agents.multi_q_sat import MultiQLearningSATCommunicative

    def communicative_main(problem):
        # Create and run communicative multi-agent solver
        solver = MultiQLearningSATCommunicative(
            problem["num_vars"], problem["clauses"], n_agents=5
        )
        solution, metrics = solver.solve(max_episodes=300, early_stopping=True)
        return metrics
...
def run_comparison(problems=None, n_runs=3, include_communicative=True):
    if problems is None:
        from src.sat_problems import MEDIUM_PROBLEM, SMALL_PROBLEM
    ...
    for problem in problems:
        print(f"\n{'='*50}")
        print(
            f"= Testing problem: {problem['name']} ({problem['num_vars']} variables, {len(problem['clauses'])} clauses)"
        )
        print(f"{'='*50}")
        ...
        for i in range(n_runs):
            ...
            if metrics["solution_found"]:
                print(
                    f"Run {i+1}: Solution found in {metrics['solution_episode']} episodes (time: {runtime:.2f}s)"
                )
            else:
...
def analyze_results(coop_metrics, comp_metrics, problem):
    ...
    coop_success = sum(1 for m in coop_metrics if m["solution_found"]) / len(
        coop_metrics
    )
    comp_success = sum(1 for m in comp_metrics if m["solution_found"]) / len(
        comp_metrics
    )
    print(
        f"Success rate: Cooperative: {coop_success*100:.1f}%, Competitive: {comp_success*100:.1f}%"
    )
    ...
    if coop_episodes:
        print(
            f"Cooperative: Avg episodes to solution: {np.mean(coop_episodes):.1f} ± {np.std(coop_episodes):.1f}"
        )
    else:
    ...
    if comp_episodes:
        print(
            f"Competitive: Avg episodes to solution: {np.mean(comp_episodes):.1f} ± {np.std(comp_episodes):.1f}"
        )
    else:
    ...
    print(
        f"Avg runtime: Cooperative: {np.mean(coop_runtime):.2f}s, Competitive: {np.mean(comp_runtime):.2f}s"
    )
    ...
    try:
        ...
        if coop_episodes and comp_episodes:
            ...
            print(
                f"Episode count t-test: p-value = {p_val:.4f} {'(significant)' if p_val < 0.05 else ''}"
            )

        # t-test for runtime
        t_stat, p_val = stats.ttest_ind(coop_runtime, comp_runtime)
        print(
            f"Runtime t-test: p-value = {p_val:.4f} {'(significant)' if p_val < 0.05 else ''}"
        )
    except ImportError:
...
def analyze_three_way(coop_metrics, comp_metrics, comm_metrics, problem):
    ...
    comm_success = sum(1 for m in comm_metrics if m["solution_found"]) / len(
        comm_metrics
    )
    print(
        f"Success rate: Cooperative: {coop_success*100:.1f}%, Competitive: {comp_success*100:.1f}%, Communicative: {comm_success*100:.1f}%"
    )
    ...
    if comm_episodes:
        print(
            f"Communicative: Avg episodes to solution: {np.mean(comm_episodes):.1f} ± {np.std(comm_episodes):.1f}"
        )
    else:
    ...
    print(
        f"Avg runtime: Cooperative: {np.mean(coop_runtime):.2f}s, Competitive: {np.mean(comp_runtime):.2f}s, Communicative: {np.mean(comm_runtime):.2f}s"
    )
    ...
    try:
        ...
        if coop_episodes and comp_episodes and comm_episodes:
            t_stat, p_val = stats.ttest_ind(coop_episodes, comp_episodes)
            print(
                f"Episode count t-test (Coop vs Comp): p-value = {p_val:.4f} {'(significant)' if p_val < 0.05 else ''}"
            )

            t_stat, p_val = stats.ttest_ind(coop_episodes, comm_episodes)
            print(
                f"Episode count t-test (Coop vs Comm): p-value = {p_val:.4f} {'(significant)' if p_val < 0.05 else ''}"
            )

            t_stat, p_val = stats.ttest_ind(comp_episodes, comm_episodes)
            print(
                f"Episode count t-test (Comp vs Comm): p-value = {p_val:.4f} {'(significant)' if p_val < 0.05 else ''}"
            )

        # t-test for runtime
        t_stat, p_val = stats.ttest_ind(coop_runtime, comp_runtime)
        print(
            f"Runtime t-test (Coop vs Comp): p-value = {p_val:.4f} {'(significant)' if p_val < 0.05 else ''}"
        )

        t_stat, p_val = stats.ttest_ind(coop_runtime, comm_runtime)
        print(
            f"Runtime t-test (Coop vs Comm): p-value = {p_val:.4f} {'(significant)' if p_val < 0.05 else ''}"
        )

        t_stat, p_val = stats.ttest_ind(comp_runtime, comm_runtime)
        print(
            f"Runtime t-test (Comp vs Comm): p-value = {p_val:.4f} {'(significant)' if p_val < 0.05 else ''}"
        )
    except ImportError:
        print("SciPy not installed - skipping statistical tests")
...
def plot_problem_results(coop_metrics, comp_metrics, problem):
    ...
    plt.suptitle(
        f"Results for {problem['name']} ({problem['num_vars']} variables, {len(problem['clauses'])} clauses)"
    )
    ...
    comp_runtime = [m["runtime"] for m in comp_metrics]
    plt.bar(
        ["Cooperative", "Competitive"], [np.mean(coop_runtime), np.mean(comp_runtime)]
    )
    plt.ylabel("Seconds")
    ...
    for i, metrics in enumerate(coop_metrics):
        if "best_reward_progress" in metrics and metrics["best_reward_progress"]:
            plt.plot(metrics["best_reward_progress"], label=f"Coop {i+1}", alpha=0.5)

    # Plot best reward progress for competitive agents
    for i, metrics in enumerate(comp_metrics):
        if "best_reward_progress" in metrics and metrics["best_reward_progress"]:
            plt.plot(
                metrics["best_reward_progress"],
                label=f"Comp {i+1}",
                alpha=0.5,
                linestyle="--",
            )
...
def plot_problem_results_three_way(coop_metrics, comp_metrics, comm_metrics, problem):
    ...
    plt.bar(
        ["Cooperative", "Competitive", "Communicative"],
        [coop_success, comp_success, comm_success],
    )
    plt.ylabel("Success Rate")
    plt.title("Solution Success Rate")
    ...
    plt.bar(
        ["Cooperative", "Competitive", "Communicative"],
        [np.mean(coop_runtime), np.mean(comp_runtime), np.mean(comm_runtime)],
    )
    plt.ylabel("Seconds")
    plt.title("Average Runtime")
...
def plot_problem_comparison(all_results):
    ...
    for name in problem_names:
        ...
        coop_success = sum(
            1 for m in results["cooperative"] if m["solution_found"]
        ) / len(results["cooperative"])
        comp_success = sum(
            1 for m in results["competitive"] if m["solution_found"]
        ) / len(results["competitive"])
        coop_success_rates.append(coop_success)
        comp_success_rates.append(comp_success)

        if "communicative" in results:
            comm_success = sum(
                1 for m in results["communicative"] if m["solution_found"]
            ) / len(results["communicative"])
