...
def download_sat_benchmarks(
    output_dir: str = "benchmarks", sources: list[dict[str, str]] | None = None
) -> None:
    """
...
def parse_cnf_header(cnf_path: str) -> tuple[list[list[int]], int]:
    ...
    with open(cnf_path, errors="ignore") as f:
        for line in f:
            line = line.strip()
...
def generate_phase_transition_problems(
    output_dir: str,
    ...
    Args:
        ...
        seed: Random seed for reproducibility
    """
    from src.sat_problems import (
        generate_sat_problem,  # Import here to avoid circular imports
    )
        ...
        for ratio in ratios:
            ...
            for instance in range(instances_per_config):
                ...
                with open(filepath, "w") as f:
                    # Write CNF header
                    f.write(
                        f"c Random {vars}-variable, {clauses}-clause 3-SAT problem\n"
                    )
            ...
            print(
                f"Generated {instances_per_config} instances with {vars} variables and ratio {ratio:.2f}"
            )
