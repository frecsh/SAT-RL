...
from src.sat_problems import is_satisfied, random_walksat
...
class MultiQLearningSATOracle(MultiQLearningSAT):
    ...
    def _update_clause_difficulty(self, state: list[int]) -> None:
        ...
        if self.clause_check_count >= 10:  # Only update periodically
            ...
            satisfaction_ratio = (
                self.clause_satisfaction_count / self.clause_check_count
            )
    ...
    def _consult_oracle(self, current_state: list[int]) -> list[int]:
        ...
        solution, solved = random_walksat(
            self.clauses, self.n_vars, max_flips=1000, random_probability=0.3
        )
            ...
            if oracle_action:
                ...
                for action in available_actions:
                    if action == oracle_action:
                        ...
                        q_values[action] = (1.0 - self.oracle_weight) * q_values.get(
                            action, 0.0
                        ) + self.oracle_weight * best_q * 1.5
    ...
    def solve(
        ...
        current_state = [
            random.choice([var, -var]) for var in range(1, self.n_vars + 1)
        ]
        ...
        for episode in range(max_episodes):
            ...
            for agent_idx in range(self.n_agents):
                ...
                self._update_q_value(
                    agent_idx, current_state, action, reward, next_state
                )
...
if __name__ == "__main__":
    ...
    # Solve
    print(
        f"Solving {n_vars}-variable, {n_clauses}-clause SAT problem with oracle guidance..."
    )
    ...
    print(
        f"Satisfaction: {stats['best_satisfied']}/{len(clauses)} clauses ({stats['best_satisfaction_ratio']:.2%})"
    )
