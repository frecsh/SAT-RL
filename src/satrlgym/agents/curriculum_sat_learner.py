...
import numpy as np
from satrlgym_logger import SATRLLogger

from src.models.improved_sat_gan import ImprovedSATGAN
from src.satrlgym.agents.anytime_sat_solver import AnytimeEnsembleSolver
from src.satrlgym.agents.deep_q_sat_agent import DeepQLearningAgent
...
class CurriculumSATLearner:
    def __init__(
        ...
        step_size=0.2,
        enable_logging=True,
        logs_dir="satrlgym_logs/curriculum",
    ):
        """
        ...
        if enable_logging:
            ...
            self.curriculum_log_path = os.path.join(
                self.logs_dir, "curriculum_progression.csv"
            )
            with open(self.curriculum_log_path, "w") as f:
                f.write(
                    "stage,ratio,episodes,success_rate,avg_reward,training_time,restart_count\n"
                )
    ...
    def get_agent_config(self, agent_type, ratio, attempt, clauses):
        ...
        if agent_type == "dqn":
            ...
            if ratio < 3.5:
                ...
                config[
                    "epsilon_decay"
                ] = 0.98  # Even slower decay near phase transition
    ...
    def solve_with_agent(
        self, agent_type, clauses, current_ratio, attempt, max_episodes=20
    ):
        ...
        starting_point = (
            self.get_diverse_starting_point()
            if hasattr(self, "solution_pool") and self.solution_pool
            else None
        )
        ...
        if agent_type == "dqn":
            ...
            agent = ImprovedSATGAN(
                ...
                initial_solutions=(
                    [starting_point] if starting_point is not None else None
                ),
            )
    ...
    def transfer_knowledge(self, new_agent, current_ratio, next_ratio):
        ...
        if (
            self.last_agent_type == "dqn"
            and not isinstance(new_agent, DeepQLearningAgent)
        ) or (
        ...
        try:
            ...
            if (
                self.last_agent_type == "dqn"
                and hasattr(self.last_agent, "model")
                ...
                for i, (sw, tw) in enumerate(zip(source_weights, target_weights)):
                    ...
                    if i < len(source_weights) - 2:
                        ...
                        blend = sw * (transfer_factor * 0.5) + tw * (
                            1 - transfer_factor * 0.5
                        )
                ...
                print(
                    f"Transferred neural network knowledge with factor {transfer_factor:.2f}"
                )
                return True

            elif (
                self.last_agent_type == "gan"
                and hasattr(self.last_agent, "experience_buffer")
                and hasattr(new_agent, "experience_buffer")
            ):
                ...
                if len(self.last_agent.experience_buffer) > 0:
                    ...
                    experiences = random.sample(
                        list(self.last_agent.experience_buffer), transfer_count
                    )
                    ...
                    print(
                        f"Transferred {transfer_count} promising solutions to new GAN buffer"
                    )
                    return True
    ...
    def get_diverse_starting_point(self):
        ...
        chosen_idx = random.choices(
            range(len(self.solution_pool)), weights=weights, k=1
        )[0]
    ...
    def visualize_learning_curve(self):
        ...
        for i, ratio in enumerate(unique_ratios):
            ...
            plt.scatter(
                ...
                [percentages[idx] for idx in indices],
                color=colors[i],
    ...
    def solve_with_curriculum(self, max_attempts=5):
        ...
        """
        print(
            f"Starting curriculum learning from ratio {self.initial_ratio} to {self.target_ratio}"
        )
        ...
        while current_ratio <= self.target_ratio:
            ...
            while not solved and attempt <= max_attempts:
                ...
                if solved:
                    print(
                        f"Solved level {level} (ratio {current_ratio:.2f}) with {agent_type}!"
                    )
                    self.best_solution = solution
            ...
            if solved:
                ...
                if percentage_solved > 0.95:  # Very close to solution
                    ...
                    print(
                        f"Close enough ({percentage_solved:.2f}), advancing to ratio {next_ratio:.2f}"
                    )
                    ...
                    print(
                        f"Making progress ({percentage_solved:.2f}), small advance to {next_ratio:.2f}"
                    )
                else:  # Struggling significantly
                    # Reduce step size but don't change ratio
                    current_step *= 0.5
                    print(
                        f"Struggling ({percentage_solved:.2f}), reducing step size to {current_step:.3f}"
                    )
        ...
        if self.enable_logging:
            # Export final traces
            final_log_path = self.logger.export_traces_to_csv(
                "curriculum_final_traces.csv"
            )
    ...
    def _log_curriculum_stage(self, stage, rewards, success_rate, training_time):
        ...
        with open(self.curriculum_log_path, "a") as f:
            f.write(
                f"{stage},{self.initial_ratio:.2f},{len(rewards)},{success_rate:.4f},"
