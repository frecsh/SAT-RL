...
class RandomAgent(AgentBase):
    ...
    def act(
        ...
        if "assignment" in observation:
            ...
            if len(assignment.shape) == 1 and assignment.shape[0] > self.num_vars:
                ...
                unassigned = [
                    i for i in range(1, self.num_vars + 1) if assignment[i] == 0
                ]
            else:
                # This might be a binary encoding - we need to check both positive and
                # negative slots
                unassigned = []
    ...
    def export_model(self) -> dict[str, Any]:
        ...
        Returns:
            Empty dictionary
        """
        return {"agent_type": "random", "config": self.agent_config}
