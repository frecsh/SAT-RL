...
class MultiQLearningSATCommunicative:
    ...
    def __init__(
        ...
        self.q_tables = [
            defaultdict(lambda: np.zeros(len(agent_vars) * 2))
    ...
    def _choose_actions(self, solution):
        ...
        for agent_idx, agent_vars in enumerate(self.agent_var_assignments):
            ...
            if len(q_values) == 0:
                ...
                self.q_tables[agent_idx][state_key] = np.zeros(
                    2
        ...
        for episode in range(max_episodes):
            ...
            for step in range(100):  # Limit steps per episode
                ...
                for agent_idx, action in agent_actions:
                    ...
                    new_difficult_clauses = self._identify_difficult_clauses(
                        new_solution
                    )
                    difficult_clauses_solved = len(
                        set(difficult_clauses) - set(new_difficult_clauses)
                    )

                    # Enhanced reward for solving difficult clauses
                    reward = (new_satisfied - old_satisfied) / len(
                        self.clauses
                    ...
                    self._update_q_value(
                        agent_idx, state_key, action, next_state_key, reward
                    )
            ...
            if (episode + 1) % 10 == 0:
                ...
                print(
                    f"Knowledge base: {len(self.knowledge_base['positive_vars'])} positive vars, "
...
if __name__ == "__main__":
    ...
    print(
        f"Testing Communicative Multi-agent Q-Learning on {n_vars}-variable, {n_clauses}-clause problem"
    )
