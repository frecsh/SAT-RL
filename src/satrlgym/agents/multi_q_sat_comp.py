    ...
    def solve(self, max_episodes=1000, early_stopping=False):
        ...
        for episode in range(max_episodes):
            ...
            for agent_idx in range(self.n_agents):
                ...
                self.agent_scores[agent_idx] = (
                    0.7 * self.agent_scores[agent_idx] + 0.3 * agent_rewards[agent_idx]
                )
            ...
            if (episode + 1) % 10 == 0:
                ...
                # Print agent scores
                print(
                    f"Agent scores: {', '.join([f'{score:.2f}' for score in self.agent_scores])}"
                )
...
if __name__ == "__main__":
    ...
    print(
        f"Testing Competitive Multi-agent Q-Learning on {n_vars}-variable, {n_clauses}-clause problem"
    )
