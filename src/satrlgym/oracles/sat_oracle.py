...
class SATOracle:
    ...
    def __init__(self, problem):
        ...
        self.clause_difficulty = np.zeros(
            len(self.clauses)
        )  # Track difficulty of each clause
        self.clause_satisfaction_count = np.zeros(
            len(self.clauses)
        )  # Track satisfaction frequency
    ...
    def critique(self, trajectory):
        ...
        satisfied_clauses = self._get_satisfied_clauses(assignment)
        unsatisfied_indices = [
            i for i in range(len(self.clauses)) if i not in satisfied_clauses
        ]
        ...
        if self.optimal_solution is not None and unsatisfied_indices:
            opt_assignment = {
                i + 1: bool(bit) for i, bit in enumerate(self.optimal_solution)
            }
...
def test_oracle():
    ...
    from satrlgym.data.test_problems import MEDIUM_PROBLEM
