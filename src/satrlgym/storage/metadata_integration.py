...
class MetadataStorageAdapter:
    ...
    def __init__(
        ...
        self.metadata_file = os.path.join(
            metadata_dir, f"{self.metadata.experiment_id}.json"
        )
        self._save_metadata()
        ...
        for exp in experiences:
            ...
            if "episode_id" in exp:
                ...
                if episode_id in self.metadata.episode_seeds:
                    exp["metadata"]["episode_seed"] = self.metadata.episode_seeds[
                        episode_id
                    ]
...
class MetadataSearchEngine:
    ...
    def __init__(
        self, metadata_dir: str = "metadata", storage_dirs: list[str] | None = None
    ):
        """
    ...
    def search_experiments(
        ...
        for experiment_id in self.list_experiments():
            ...
            if (
                ...
                and description_contains.lower() not in metadata.description.lower()
            ):
                continue
            ...
            if custom_metadata_filter:
                ...
                for key, value in custom_metadata_filter.items():
                    if (
                        key not in metadata.custom_metadata
                        or metadata.custom_metadata[key] != value
                    ):
                        match = False
                        break
    ...
    def _find_storage_for_experiment(self, experiment_id: str) -> StorageBackend | None:
        """
        ...
        for storage_dir in self.storage_dirs:
            ...
            if os.path.exists(arrow_dir):
                try:
                    ...
                    logger.debug(
                        f"Failed to load Arrow storage for {experiment_id}: {e}"
                    )
            ...
            if os.path.exists(hdf5_file):
                try:
                    ...
                    logger.debug(
                        f"Failed to load HDF5 storage for {experiment_id}: {e}"
                    )
    ...
    def reproduce_episode(
        self, experiment_id: str, episode_id: int
        ...
        if not metadata or episode_id not in metadata.episode_seeds:
            logger.warning(
                f"No seed found for episode {episode_id} in experiment {experiment_id}"
            )
            return None
        ...
        episode_experiences = [
            exp for exp in all_experiences if exp.get("episode_id") == episode_id
        ]
...
def load_storage_with_metadata(
    experiment_id: str,
    storage_dirs: list[str] | None = None,
    metadata_dir: str = "metadata",
) -> tuple[MetadataStorageAdapter, ExperimentMetadata] | None:
    """
