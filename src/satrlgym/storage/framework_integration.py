...
from ..type_system import TypeSpecification
...
if TORCH_AVAILABLE:
    ...
    class ExperienceMapDataset(Dataset):
        ...
        def __init__(
            self, backend: StorageBackend, transform=None, max_cache_size: int = 1000
        ):
            """
    ...
    class ExperienceTensorflowDataset:
        ...
        def create_tf_dataset(
            ...
            for field_name, value in sample.items():
                (
                    output_types[field_name],
                    output_shapes[field_name],
                ) = self._get_tf_type_shape(value)
            ...
            if isinstance(value, dict):
                ...
                for k, v in value.items():
                    (
                        types[k],
                        shapes[k],
                    ) = ExperienceTensorflowDataset._get_tf_type_shape(v)
                return types, shapes
        ...
        def _value_to_tf_feature(
            self, name: str, value: Any, spec: TypeSpecification | None = None
            ...
            if isinstance(value, dict):
                ...
                for k, v in value.items():
                    ...
                    sub_features = self._value_to_tf_feature(
                        f"{name}/{k}", v, nested_spec
                    )
                ...
                if np.issubdtype(value.dtype, np.floating):
                    ...
                    features[name] = tf.train.Feature(
                        int64_list=tf.train.Int64List(
                            value=value.flatten().astype(np.int64)
                        )
                    )
...
def batch_to_torch(
    batch: list[dict[str, Any]], device: str = "cuda"
...
def normalize_observations(
    batch: dict[str, Any], stats: dict[str, dict[str, float]]
) -> dict[str, Any]:
    """
    ...
    if "observation" in batch:
        ...
        if isinstance(obs, dict):
            for key, value in obs.items():
                if key in stats and isinstance(
                    value, (np.ndarray, torch.Tensor, tf.Tensor)
                ):
                    mean = stats[key]["mean"]
                    ...
                    if isinstance(value, np.ndarray):
                        ...
                        batch["observation"][key] = (
                            value - torch.tensor(mean, device=value.device)
        ...
        # Handle array observations
            ...
            if "observation" in stats:
                ...
                if isinstance(obs, np.ndarray):
                    ...
                    batch["observation"] = (
                        obs - torch.tensor(mean, device=obs.device)
...
def calculate_n_step_returns(
    batch: list[dict[str, Any]], gamma: float = 0.99, n_steps: int = 1
) -> list[dict[str, Any]]:
    """
...
def load_mmap_array(
    filename: str, shape: tuple, dtype: np.dtype, mode: str = "r+"
) -> np.ndarray:
    """
