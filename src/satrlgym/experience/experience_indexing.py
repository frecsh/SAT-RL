...
class ExperienceIndex:
    ...
    def get_transition(self, index: int) -> dict[str, Any]:
        ...
        if index < 0 or index >= self.transitions_count:
            raise IndexError(
                f"Index {index} out of bounds (0-{self.transitions_count-1})"
            )
    ...
    def _get_transition_parquet(self, index: int) -> dict[str, Any]:
        ...
        try:
            ...
            table = pq.read_table(
                self.source_file, row_groups=[index // 1000]
            )  # Assuming default row group size
    ...
    def sample(self, count: int = 1, prioritized: bool = False) -> list[dict[str, Any]]:
        ...
        if count > self.transitions_count:
            raise ValueError(
                f"Cannot sample {count} from {self.transitions_count} transitions"
            )
...
class PrioritizedReplayBuffer:
    ...
    def add(self, experience: dict[str, Any], priority: float | None = None):
        """
    ...
    def sample(
        ...
        if np.sum(active_priorities) == 0 or not np.any(active_priorities > 0):
            ...
            indices = np.random.choice(
                self._size, batch_size, replace=False, p=probabilities
            )
