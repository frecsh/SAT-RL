...
class StructuredLogger:
    ...
    def _get_file(self, event_type: str) -> tuple:
        ...
        if event_type not in self.files:
            ...
            file = open(filepath, "w", newline="" if self.format_type == self.FORMAT_CSV else None)
    ...
    def log_reward(self, episode: int, step: int, reward: float, info: Optional[Dict] = None):
        """
        Log a reward event.
    ...
    def log_clause_stats(self, episode: int, step: int, satisfied_count: int, total_count: int):
        """
    ...
    def log_agent_decision(self, episode: int, step: int, agent_id: str, action: Any, 
                          observation: Dict, action_probs: Optional[Dict] = None):
        """
    ...
    def log_exception(self, episode: int, step: int, exception_type: str, 
                     exception_message: str, stack_trace: str):
        """
        Log an exception.
    ...
    def finalize(self) -> str:
        ...
        if self.visualize_ready:
            ...
            # Write metadata file
            metadata_path = os.path.join(self.output_dir, f"{self.experiment_name}_viz_metadata.json")
...
def create_logger(experiment_name: str, output_dir: str = "logs", 
                 format_type: str = "json", visualize_ready: bool = False) -> StructuredLogger:
    """
