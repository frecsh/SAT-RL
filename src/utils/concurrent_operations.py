...
class ConcurrentWriter:
        ...
        try:
            ...
            if self.lock is not None:
                if self.lock.is_locked:
                    self.lock.release()
                self.lock = None
            raise IOError(f"Could not open file {self.base_filepath}: {e}")
    ...
    def _write_parquet(self, data: Dict[str, Any]):
        ...
        try:
            ...
            raise ImportError("pandas and pyarrow are required for Parquet file support")
...
class ShardedWriter:
    ...
    def get_shard_writer(self, shard_id: int) -> ConcurrentWriter:
        ...
        if shard_id < 0 or shard_id >= self.num_shards:
            raise ValueError(f"Invalid shard ID: {shard_id}, must be 0-{self.num_shards-1}")
        
        # Create the writer if it doesn't exist
        if shard_id not in self.writers:
            shard_path = self.base_directory / self.metadata["shards"][str(shard_id)]["path"]
    ...
    def write(self, data: Dict[str, Any], shard_id: Optional[int] = None) -> int:
        """
    ...
    def _select_shard(self) -> int:
        ...
        with self.metadata_lock:
            # Find active shards
            active_shards = [
                int(shard_id) for shard_id, info in self.metadata["shards"].items()
                if info["active"]
            ]
            ...
            return min(
                active_shards,
                key=lambda s: self.metadata["shards"][str(s)]["records"]
            )
    ...
    def merge_shards(self, output_path: Optional[Union[str, Path]] = None) -> Path:
        """
                    ...
                    with h5py.File(full_path, "r") as in_f:
                        for key in in_f.keys():
                            if key in out_f:
                                ...
                                new_shape = (current_size + in_data.shape[0],) + out_data.shape[1:]
    ...
    def _read_npz(self, index: int) -> Dict[str, Any]:
        ...
        with FileLock(str(self.lock_file), timeout=self.lock_timeout):
            with np.load(self.filepath, allow_pickle=True) as data:
                ...
                for key in data.files:
                    if len(data[key]) > index:
                        result[key] = data[key][index].item() if data[key][index].ndim == 0 else data[key][index]
        ...
        with FileLock(str(self.lock_file), timeout=self.lock_timeout):
            with h5py.File(self.filepath, "r") as f:
                ...
                for key in f.keys():
                    if isinstance(f[key], h5py.Dataset) and len(f[key].shape) > 0:
                        if f[key].shape[0] > index:
                            result[key] = f[key][index].item() if f[key][index].ndim == 0 else f[key][index]
