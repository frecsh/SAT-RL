...
class HDF5StorageBackend(StorageBackend):
    ...
    def _initialize_storage(self) -> None:
        ...
        if not file_exists:
            ...
            exp_group = self.file.create_group('experiences')
    ...
    def _get_field_datasets(self, chunk_idx: int) -> Dict[str, h5py.Dataset]:
        ...
        for field_name, field_spec in self.schema.field_specs.items():
            if field_name not in chunk_group:
                ...
                dataset = self._create_field_dataset(chunk_group, field_name, field_spec)
            else:
    ...
    def _create_field_dataset(self, group: h5py.Group, field_name: str, 
                              field_spec: TypeSpecification) -> h5py.Dataset:
        """
        ...
        # Handle array types
            ...
            chunks[0] = min(self.chunk_size, 100)  # Chunk size for experiences dimension
    ...
    def _dtype_to_hdf5_dtype(self, dtype: DType) -> np.dtype:
        ...
        type_map = {
            ...
            DType.BOOL: np.bool_,
            DType.STRING: np.dtype('O')  # Will be converted to h5py.special_dtype later
        }
    ...
    def write_batch(self, experiences: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            experiences_in_chunk = min(len(experiences), self.chunk_size - local_start_idx)
            
            # Write experiences to current chunk
            if experiences_in_chunk > 0:
                self._write_to_chunk(chunk_idx, local_start_idx, 
                                     experiences[:experiences_in_chunk])
    ...
    def _write_to_chunk(self, chunk_idx: int, local_start_idx: int, 
    ...
    def _write_field_values(self, dataset_or_group, values: List[Any], 
                            spec: TypeSpecification, start_idx: int) -> None:
        """
        ...
        if spec.nested_spec:
            for nested_name, nested_spec in spec.nested_spec.items():
                ...
                nested_values = [v[nested_name] if v is not None else None for v in values]
                
                # Write to nested dataset or group
                self._write_field_values(
                    dataset_or_group[nested_name],
                    nested_values,
                    nested_spec,
                    start_idx
                )
    ...
    def read_batch(self, batch_size: int = 32, indices: Optional[List[int]] = None) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if indices is not None:
                ...
                for global_idx, (chunk_idx, local_idx) in zip(indices, chunk_local_indices):
    ...
    def _get_chunk_local_indices(self, global_indices: List[int]) -> List[Tuple[int, int]]:
        """
        ...
        for global_idx in global_indices:
            if 0 <= global_idx < self.total_experiences:
                ...
                logger.warning(f"Index {global_idx} out of range [0, {self.total_experiences})")
                # Use default values
                result.append((0, 0))
        
        return result
    
    def _read_from_chunk(self, chunk_idx: int, local_indices: List[int]) -> List[Dict[str, Any]]:
        """
    ...
    def _read_field_values(self, dataset_or_group, spec: TypeSpecification, 
...
def create_hdf5_storage(schema: ExperienceSchema, path: str, **kwargs) -> HDF5StorageBackend:
    """
